...
class Posts:
    ...
    def login(self):
        ...
        try:
            button = self.wait.until(EC.presence_of_element_located((By.XPATH,
                                                                     "/html/body/div/div/div/div[1]/div/div/div/div/div/div/div[2]/div[2]/div/div/div[2]/div[2]/div/div/div/button[2]")))
        ...
        try:
            button = self.wait.until(EC.presence_of_element_located((By.XPATH,
                                                                 "/html/body/div/div/div/div[1]/div/div/div/div/div/div/div[2]/div[2]/div/div/div[2]/div[2]/div[2]/div/div[1]/div/div/button")))
    ...
    def extract_tweets_from_html(self, file_path):
        ...
        # Trova tutti i tweet nel file HTML
        tweet_blocks = re.findall(r'<article.*?data-testid="tweet".*?</article>', content, re.DOTALL)
        
        for tweet in tweet_blocks:
            try:
                # Estrai username
                username_match = re.search(r'href="/(.*?)"', tweet)
                username = username_match.group(1) if username_match else "Unknown"
                
                # Estrai tweet_id
                tweet_id_match = re.search(r'status/([0-9]+)', tweet)
                tweet_id = tweet_id_match.group(1) if tweet_id_match else "Unknown"
                
                # Estrai testo del tweet
                tweet_text_match = re.search(r'data-testid="tweetText".*?>(.*?)</div>', tweet, re.DOTALL)
                tweet_text = re.sub(r'<.*?>', '', tweet_text_match.group(1)) if tweet_text_match else "Unknown"
                
                # Estrai link alla discussione
                discussion_link = f"https://x.com/{username}/status/{tweet_id}"
                
                # Estrai timestamp ISO 8601
                timestamp_match = re.search(r'<time datetime="(.*?)">', tweet)
                iso_timestamp = timestamp_match.group(1) if timestamp_match else "Unknown"
                
                # Converte il timestamp in datetime
                datetime_timestamp = datetime.fromisoformat(iso_timestamp.replace('Z', '')) if iso_timestamp != "Unknown" else "Unknown"
                print(tweet_id)
                if tweet_id not in self.tweets_ids:
                    self.tweets.append({
                        "username": username,
                        "tweet_id": tweet_id,
                        ...
                        "datetime_timestamp": str(datetime_timestamp)
                    })
                    self.tweets_ids.append(tweet_id)
            except Exception as e:
                print(f"Errore nell'estrazione di un tweet: {e}")
    ...
    def complete_search(self):
        ...
        while True:
            ...
            # Save raw_tweets to a file
            with open('raw_tweets.txt', 'w') as file:
                file.write(str(self.raw_tweets))
            
            self.extract_tweets_from_html('raw_tweets.txt')
